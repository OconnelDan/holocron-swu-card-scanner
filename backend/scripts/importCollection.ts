...
async function importCollection() {
  try {
    ...
    const missingFields = requiredFields.filter(field => 
      !headers.some(h => normalizeHeader(h) === normalizeHeader(field))
    );
        ...
        if (totalQuantity > 0) {
          if (processedCards.has(swudbId)) {
            ...
            Object.keys(variants).forEach(key => {
              existingCard.variants[key] += variants[key];
            });
    ...
    // Agrupar cartas por "Base card id" y sumar cantidades
    const cardGroups = new Map<string, CollectionCard>();
    
    data
      .filter(row => row.Name && row['Base card id']) // Filtrar filas válidas
      .forEach(row => {
        const baseCardId = row['Base card id']?.toString() || '';
        const name = row.Name || '';
        const set = row.Set || '';
        
        // Calcular cantidad total sumando las variantes disponibles
        const availableQuantities = ['Normal', 'Foil', 'Hyperspace', 'Foil & Hyperspace', 'Showcase',
          'Organized Play', 'Event Exclusive', 'Prerelease Promo', 
          'Organized Play Foil', 'Standard Prestige', 'Foil Prestige', 
          'Serialized Prestige'];
        
        // Solo sumar las columnas que realmente existen en el archivo
        const totalQuantity = availableQuantities.reduce((sum, variant) => {
          if (row.hasOwnProperty(variant)) {
            const qty = parseInt(row[variant]) || 0;
            return sum + qty;
          }
          return sum;
        }, 0);
        ...
        // Solo procesar si tiene cantidad > 0
        if (totalQuantity > 0) {
          if (cardGroups.has(baseCardId)) {
            // Si ya existe, sumar cantidad
            const existingCard = cardGroups.get(baseCardId)!;
            ...
            if (row.Rarity) card.rarity = row.Rarity;
            if (row.Type) card.type = row.Type;
            if (row.Cost) card.cost = parseInt(row.Cost);
            if (row.Power) card.power = parseInt(row.Power);
            if (row.HP) card.hp = parseInt(row.HP);
            
            cardGroups.set(baseCardId, card);
          }
        }
      });
    ...
    console.log(`✅ Procesadas ${cards.length} cartas válidas`);
    ...
    console.log(`   - Total de cartas físicas: ${stats.totalQuantity}`);
    console.log(`   - Sets: ${stats.sets.join(', ')}`);
    console.log(`   - Rarezas: ${stats.rarities.join(', ')}`);
